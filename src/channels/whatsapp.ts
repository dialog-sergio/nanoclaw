...
export class WhatsAppChannel implements Channel {
          ...
          if (groups[chatJid]) {
            ...
            if (normalized?.documentMessage?.mimetype === 'application/pdf') {
              try {
                ...
                const pdfCaption = normalized.documentMessage.caption || '';
                content = pdfCaption ? `${pdfCaption}\n\n${pdfRef}` : pdfRef;
                logger.info(
            ...
            // Skip protocol messages with no text content, but allow voice messages
  ...
  private async getNormalizedGroupMetadata(
    ...
    logger.info(
      { jid, participantCount: participants.length },
